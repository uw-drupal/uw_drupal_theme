...
UW.Select = Backbone.View.extend({
	...
	close: function( e ) {
		...
		let selected = this.$target.index();
...
return;
	...
	atEdge: function( direction ) {
		return ( 0 === this.current && 'up' === direction ||
		this.current === this.$el.find( 'li' ).length - 1 && 'down' === direction );
	},
